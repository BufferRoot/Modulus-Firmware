...
static void process_rx_pkt(interface_buffer_handle_t *buf_handle)
{
	...
	if (buf_handle->if_type == ESP_STA_IF && station_connected) {
		...
		espnow_process_host_cmd(payload, payload_len);
	}
	else if (buf_handle->if_type == ESP_ZIGBEE_IF) {
		zigbee_process_host_cmd(payload, payload_len);
	}
	else if (buf_handle->if_type == ESP_THREAD_IF) {
		thread_process_host_cmd(payload, payload_len);
	}
